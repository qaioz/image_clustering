...
@pytest.mark.parametrize("centroids, elements_per_centroid, norm, expected_cost", [
    (
        np.array([[1, 1, 1], [0, 10, 10]]), 
        [np.array([[1, 4, 5], [2, 3, 3]]), np.array([[0, 10, 10], [1, 8, 8], [0, 10, 5]])],
        2, 
        16
    )
])
...
# norm = 2
# expected_0 = [[1,1,1]]
# expected_1 = [[10,10,10] 4 times, [15,15,15]]
# expected_2 = [[100,100,100] 3 times]
@pytest.mark.parametrize("image, centroids, norm, expected", [
    (
        np.array([
        ...
        [
            np.array([[1,1,1]]),
            np.array([[15,15,15], [10,10,10], [10,10,10], [10,10,10], [10,10,10]]),
            np.array([[100,100,100], [100,100,100], [100,100,100]])
        ]
    
    )
])
def test_partition(image, centroids, norm, expected):
    partition = kmeans.partition(image, centroids, norm)
    
    for i in range(len(partition)):
        sorted_partition = np.sort(partition[i], axis=0)
        sorted_expected = np.sort(expected[i], axis=0)
...
@pytest.mark.parametrize("elements_per_centroid, expected", [
    (
        [np.array([[1,1,1],[3,3,3]]), np.array([[10,10,10],[12,12,12], [14,14,14]])],
...
@pytest.mark.parametrize("num_of_vectors_1, num_of_vectors_2, error_range", [
    (10**6, 10**7, 0.8),
    (10**6, 10**7, 0.7)
]
)
def test_generate_new_centroids_probailistic_large_values(num_of_vectors_1, num_of_vectors_2, error_range):
    """
    ...
    elements_per_centroid = [np.random.randint(0, 255, (num_of_vectors_1, 3)), np.random.randint(0, 255, (num_of_vectors_2, 3))]
